...
with st.container():
    ...
    with col1:
        st.image("/Users/vaidhav/Downloads/Ruby Williams.png", width=100, caption="ResWhisper Icon")
    with col2:
...
if st.session_state["page"] == "login":
    ...
    if st.button("Login"):
        if username in st.session_state["users_db"] and st.session_state["users_db"][username]["password"] == password:
            ...
            st.session_state["resume_data"] = st.session_state["users_db"][username].get("data", st.session_state["resume_data"])
    ...
    if st.button("Sign Up"):
        if new_username in st.session_state["users_db"]:
            ...
            st.session_state["users_db"][new_username] = {"password": new_password, "data": st.session_state["resume_data"]}
            st.session_state["authenticated"] = True
    ...
    **Key Features:**
    ...
    with col2:
        ...
        if st.session_state["username"]:
            st.session_state["users_db"][st.session_state["username"]]["data"] = resume_data
    ...
    if st.button("🔄 Start Over", key="start_over"):
        for key in list(st.session_state.keys()):
            if key != "consent_given" and key != "authenticated" and key != "username" and key != "users_db":
                if key == "page":
